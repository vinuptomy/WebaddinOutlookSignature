...
export async function getGraphData() {
  try {
    ...
    if (exchangeResponse.error) {
      ...
      const userprofileresponse = await sso.makeGraphApiCall(exchangeResponse.access_token);
      documentHelper.writeDataToOfficeDocument(userprofileresponse);
      const spdataresponse = await sso.makeGraphApiCallSP(exchangeResponse.access_token);

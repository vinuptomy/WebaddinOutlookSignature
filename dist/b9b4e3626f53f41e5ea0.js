...
function is_not_valid_email_address(email_address) {
  var email_address_regex = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
...
function create_user_info() {
  var email = $("input#PlaceHolderMain_txtEmail").val().trim();
  var name = $("input#PlaceHolderMain_txtName").val().trim();
  clear_message();
...
function clear_roaming_settings() {
  ...
  Office.context.roamingSettings.saveAsync(function (asyncResult) {
    ...
    var message = "All settings reset successfully! This add-in won't insert any signatures. You can close this pane now.";
